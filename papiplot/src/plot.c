...
/*
void get_nb_of_actions_in_actor(char* path, event_acum_per_action_* acum_per_action){
	acum_per_action = malloc(sizeof(event_acum_per_action_));
	acum_per_action->actions_nb = 0;
	char* token;
	char buf[1500];
	...
	while(fgets(buf,1500,ofile)!=NULL){
		...
		if(token!=NULL){
			...
			if (found == -1){
				printf("not found, adding action %s\n", token);
				acum_per_action->action_name[acum_per_action->actions_nb] = malloc(strlen(token)+1);
				strcpy(acum_per_action->action_name[acum_per_action->actions_nb],token);

				acum_per_action->actions_nb++;
			}
			else {
				//already exists, sum acum on mark i
				found = -1;
			}
		}
	}
	...
	int j;
	for(j = 0; j<=acum_per_action->actions_nb; j++){
